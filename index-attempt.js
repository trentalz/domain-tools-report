...
async function iterateOverURLs() {
    ...
    for (var i = 0; i < urlList.length; i++) {
        ...
        let expiryDateResult = await checkExpiry(url);
...
function checkExpiry(url) {
    //make a promise to handle the async call 
    return new Promise((resolve, reject) => {
        try {
            const req = https.get(url, res => {
                ...
                // when we get the data we want, send it back to whatever called it
                resolve(expiryDate);
